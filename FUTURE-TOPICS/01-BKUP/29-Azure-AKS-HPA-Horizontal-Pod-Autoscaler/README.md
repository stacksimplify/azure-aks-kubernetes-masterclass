...
- This command creates an autoscaler that targets 30 percent CPU utilization for the deployment, with a minimum of one pod and a maximum of ten pods. 
...
- When the load is greater than 30 percent, the autoscaler tries to increase the number of pods in the deployment, up to a maximum of ten
```
# Template
kubectl autoscale deployment <deployment-name> --cpu-percent=30 --min=1 --max=10

# Replace
kubectl autoscale deployment hpa-demo-deployment --cpu-percent=30 --min=1 --max=10
...
- Once CPU utilization of pods is less than 30%, it will starting terminating pods and will reach to minimum 1 pod as configured.
